...
// Make terminal script reactive to language changes
const terminalScript = computed(() => [
  { id: 1, delay: 200, text: t('install.script.info'), type: 'info', mobileHide: false },
  ...
  { id: 5, delay: 1100, text: t('install.script.success'), type: 'highlight', mobileHide: true },
  { id: 6, delay: 1300, text: '', type: 'empty', mobileHide: false },
  { id: 7, delay: 1500, text: t('install.script.final'), type: 'final', mobileHide: false },
...
useIntersectionObserver(
  ...
  ([{ isIntersecting }]) => {
    if (isIntersecting && !isVisible.value) {
      ...
      // Trigger animation
      terminalScript.value.forEach(line => {
        setTimeout(() => {
...
<template>
  <section ref="targetRef" class="w-full flex flex-col items-center py-[80px] md:py-[120px] px-4 md:px-8 relative bg-background">
    ...
    <div class="flex flex-col items-center w-full max-w-[700px] transition-all duration-1000 ease-out"
      ...
      <!-- Code installation box (Flat Atom) -->
      ...
      <!-- Terminal animation block (Subtle Borderless Canvas) -->
      ...
      <div class="w-full text-left p-6 md:p-8 rounded-lg border border-border bg-muted/10 font-mono text-sm md:text-[15px] leading-relaxed shadow-none min-h-[160px] md:min-h-[280px]">
        <div v-for="line in terminalScript" :key="line.id"
             :class="{
               'hidden md:block': line.mobileHide, // Extreme Mobile Truncation
          ...
          <!-- Syntax highlighting palette (Brand Monochrome + Green) -->
          ...
          <template v-else-if="line.type === 'highlight'">
            <span class="text-emerald-500 font-medium">{{ line.text.split(':')[0] }}:</span>
            <span class="text-muted-foreground">{{ line.text.substring(line.text.indexOf(':') + 1) }}</span>
          </template>
          
          <template v-else-if="line.type === 'final'">
            <span class="text-emerald-500 font-medium">{{ line.text }}</span>
          </template>
